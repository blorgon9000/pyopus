...
static PyObject * pypvm_config (PyObject *self, PyObject *args, PyObject * keywords) {
  ...
  // free(hinfo);	// Should not release this (belongs to libpvm) Arpad Buermen
  return (fctval);
}
...
"zeros.  However, if any of them are negative numbers,  they are indicative"
"that the addhost operation failed on that corresponding\n"
...
static PyObject * pypvm_addhosts(PyObject *self, PyObject *args, PyObject *keywords) {
    ...
    if (!PyArg_ParseTuple(args, "O", &host_list))
...
static PyObject * pypvm_pkbyte(PyObject* self, PyObject* args, PyObject *keywords) {
  ...
  if ((chars = (char *) PyMem_Malloc(list_len * sizeof(char))) < 0)
      return (NULL);
  for (i = 0; i < list_len; i++) {
    /* extract chars from list */
    item = PyList_GetItem(char_list, i);
    if ((chars[i] = PyString_AsString(item)[0]) < 0
	|| PyString_Size(item) > 1) {
      /* not a list of strings or string is more than one character */
      PyErr_SetString(PyExc_TypeError,
		      "argument 1: expected list of chars");
      PyMem_Free(chars);
      return (NULL);
    }
  }
...
static char pypvm_pkstr__doc[] = "pypvm.pkstr(str) packs the string [str]";
...
static char pypvm_pkbytestr__doc[] = 
...
static PyObject * pypvm_pkbytestr(PyObject * self, PyObject * args, PyObject * keywords) {
  char *str;
  unsigned int count;
  int info;
  char * kwlist[] = { "str" , NULL };
  if (!PyArg_ParseTupleAndKeywords(args, keywords, "s#", kwlist, &str, &count))
    /* ASSERT: incorrect arguments */
    return (NULL);
  info = pvm_pkuint(&count, 1, 1);
  if (info<0) {
    return_none(info);
  }
  Py_BEGIN_ALLOW_THREADS
  info = pvm_pkbyte(str, count, 1);
  Py_END_ALLOW_THREADS
  return_none(info);
}
...
static char pypvm_upkbytestr__doc[]=
"pypvm.upkbytestr() unpacks a byte string from the active receive buffer\n"
"NULL characters are allowed in the string.";
 /* " */

static PyObject *pypvm_upkbytestr(PyObject *self,PyObject *args, PyObject *keywords) {
  PyObject * resultobj;
  int info; 
  unsigned int count; 
  char * buffer;   

  info = pvm_upkint(&count, 1, 1);
  if (info<0) {
    return NULL;
  } else {  
    buffer = (char *) PyMem_Malloc(count * sizeof(char) + 1);
    if (buffer==NULL) { return NULL; }
    Py_BEGIN_ALLOW_THREADS
    info = pvm_upkbyte(buffer, count, 1);
    Py_END_ALLOW_THREADS
    if (info<0)
      resultobj=NULL;
    else
      resultobj = PyString_FromStringAndSize(buffer, count); 
    PyMem_Free(buffer);
  }
  return resultobj;
}
...
static PyMethodDef pvmMethods[] = {
 ...
 { "pkbyte", (PyCFunction) pypvm_pkbyte, METH_VARARGS|METH_KEYWORDS, pypvm_pack_by_type__doc },
 { "pkint", (PyCFunction) pypvm_pkint, METH_VARARGS|METH_KEYWORDS, pypvm_pack_by_type__doc },
 { "pkdouble", (PyCFunction) pypvm_pkdouble, METH_VARARGS|METH_KEYWORDS, pypvm_pack_by_type__doc },
 { "pkstr", (PyCFunction) pypvm_pkstr, METH_VARARGS|METH_KEYWORDS, pypvm_pack_by_type__doc },
 { "pkbytestr", (PyCFunction) pypvm_pkbytestr, METH_VARARGS|METH_KEYWORDS, pypvm_pkbytestr__doc },
 { "pklong", (PyCFunction) pypvm_pklong, METH_VARARGS|METH_KEYWORDS, pypvm_pack_by_type__doc },
 { "pkshort", (PyCFunction) pypvm_pkshort, METH_VARARGS|METH_KEYWORDS, pypvm_pack_by_type__doc },
 ...
 { "upkbytestr", (PyCFunction) pypvm_upkbytestr, METH_VARARGS|METH_KEYWORDS, pypvm_upkbytestr__doc },
...
void init_pypvm_core_internal(const char *modulename) 
{
  ...
  pypvm_module = Py_InitModule(modulename, pvmMethods);
...
__declspec(dllexport) void init_pypvm_core() {
  init_pypvm_core_internal("_pypvm_core");
}

__declspec(dllexport) void init_pypvm_core_i386() {
  init_pypvm_core_internal("_pypvm_core_i386");
}

__declspec(dllexport) void init_pypvm_core_amd64() {
  init_pypvm_core_internal("_pypvm_core_amd64");
}
