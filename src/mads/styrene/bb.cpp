...
// Define our own round function (MSVC 2008 does not provide it) AB
#ifdef WINDOWS
#include <math.h>
...
// int main_sty ( int argc , char ** argv ) {
extern "C" int sty_itf(double *xin, double *fout, double *gout) {
  double g0  = 1e+20;
  double g1  = 1e+20;
  double g2  = 1e+20;
  double g3  = 1e+20;
  ...
  */
  ...
  // arrondis :
  if (ARRONDI) {
    // changed round to std::round (for Windows compile) AB
    y[12] = round(y[12]);
    y[13] = round(y[13]);
...
//     cout << "y[" << i << "] = " << y[i] << endl;
  ...
  *fout=f;
  ...
  if ( g0  < eps &&
    ...
    double best_z       = 1e+20;
    char   file_name [] = "/home/sebld/DOCTORAT/GPS/NOMADr/problems/ProcesSim_v3/black_box/truth/best_x.txt";
    in.open(file_name);
    if (!in.fail()) {
      in >> best_z;
      in.close();
